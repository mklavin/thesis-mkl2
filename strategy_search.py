...
def put_together_preprocess_search(data, conc, region:str):
    ...
    len_permutations = [3] # can change this for more combos if needed
    # total_perm = []
    ...
    for n in total_perm:
        ...
        for i in n:
            ...
            if i == 'baseline removal':
                df, fit = baseline_search(df, conc)
                fits.append(fit)
    ...
    print(str(total_perm[max_index]), max(results)) # the best model with the best correlation

    for i in total_perm[max_index]:
        if i == 'normalizing':
            df = normalize(df)
        if i == 'standardize':
            df = scale_rows_to_max(df, region)
...
def baseline_search(df, conc):
    ...
    func = [pybaselines.whittaker.asls, pybaselines.whittaker.iasls, pybaselines.whittaker.airpls,
            ...
            pybaselines.smooth.noise_median, pybaselines.smooth.snip,
...
if __name__ == '__main__':
    ...
    #df.to_csv('data/prepro_580.csv', index=False)
